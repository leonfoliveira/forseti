...
import {
  JuryContextProvider,
  useJuryContext,
} from "@/app/contests/[slug]/jury/_context/jury-context";
...
describe("JuryContextProvider", () => {
  ...
  it("should load contest data and render children", async () => {
    ...
    const { result } = renderHook(() => useJuryContext(), {
      wrapper: ({ children }) => (
        <JuryContextProvider>
          <span data-testid="child" />
          {children}
  ...
  it("should connect and disconnect to listener", async () => {
    const { unmount } = renderHook(() => useJuryContext(), {
      wrapper: ({ children }) => (
