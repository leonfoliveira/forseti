...
class ContestLeaderboardController(
    ...
    val freezeLeaderboardUseCase: FreezeLeaderboardUseCase,
) {
    ...
    @Operation(summary = "Freeze contest leaderboard by id")
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "200", description = "Contest leaderboard frozen successfully"),
            ApiResponse(
                responseCode = "403",
                description = "Forbidden",
                content = [Content(mediaType = "application/json", schema = Schema(implementation = ErrorResponseDTO::class))],
    ...
    fun freeze(
        ...
        val member = RequestContext.getContext().session!!.member
        freezeLeaderboardUseCase.freeze(contestId, member.id)
        return ResponseEntity.ok().build()
    }

    @PutMapping("/contests/{contestId}/leaderboard:unfreeze")
    @Private(Member.Type.ADMIN)
    @Operation(summary = "Unfreeze contest leaderboard by id")
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "200", description = "Contest leaderboard unfrozen successfully"),
            ApiResponse(
                responseCode = "403",
                description = "Forbidden",
                content = [Content(mediaType = "application/json", schema = Schema(implementation = ErrorResponseDTO::class))],
    ...
    fun unfreeze(
        ...
        val member = RequestContext.getContext().session!!.member
        freezeLeaderboardUseCase.unfreeze(contestId, member.id)
        return ResponseEntity.ok().build()
    }
}
