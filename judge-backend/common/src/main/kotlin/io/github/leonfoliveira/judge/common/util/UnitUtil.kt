...
object UnitUtil {
    private val cpuMultipliers = mapOf(
        "m" to 1_000_000L,
        "" to 1_000_000_000L,
    )

    private val memoryMultipliers = mapOf(
        "" to 1L,
        "B" to 1L,
        "K" to 1_000L,
        "M" to 1_000_000L,
    ...
    private val timeMultipliers = mapOf(
        "" to 1L,
        "ms" to 1L,
        "s" to 1_000L,
        "m" to 60_000L,
    ...
    private fun parseValue(value: String, multipliers: Map<String, Long>): Long {
        val unit = value.takeLastWhile { it.isLetter() }
        val number = value.dropLast(unit.length).toDoubleOrNull()
            ?: throw BusinessException("Invalid unit format: $value")
        val multiplier = multipliers[unit]
